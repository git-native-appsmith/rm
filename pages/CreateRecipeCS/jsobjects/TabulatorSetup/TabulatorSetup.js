...
/*# sourceMappingURL=tabulator.min.css.map */
			...
			<div id="product-div" class="modal" style="position:absolute; top: 5%; left: 5%; max-width: 100%!important">
				<div style="display: flex; justify-content: space-between; background-color: #eaeaea; padding: 10px;">
					<div>
						<label id="GSMLabel" for="GSM">GSM:</label>
						<div>
							<input type="number" placeholder="Start" id="fromGSM" value="1" min="1" max="9999">
							<input type="number" placeholder="End" id="toGSM" min="1" max="9999">
						</div>
					</div>
					<div>
						<label id="GradeLabel" for="Grade">Grade:</label>
						<div>
							<select id="grade">
								<option value="NA">Choose a grade</option>
							</select>
						</div>
					</div>
					<div>
						<button id="searchButton" class="button" onclick="getProductCodes()">Search</button>
					</div>
				</div><br>
				<div id="iFrame-product-table"></div>
			</div>
			<script>				
				let propertyTable;
				...
				function getPropertyColumnsConfig() {
					var columnsConfig =  {
						columns: [
							...
							{ title: i18next.t("LIMIT SHO"), field: "MATERIAL_NAME_SHORT", headerFilter: true},
				...
				function setupMaterialTabulator(rowData, tableData) {
					...
					materailTable = new Tabulator("#iFrame-material-table", {
						...
						ajaxResponse: function (url, params, response) {
							let last_page = 1;
							return {
								data: rowData.RECIPE_FUNCTION == 'PE' ? response.filter( x => x.MATERIAL_ORIGIN == 4) : response,
								last_page
							};
						},
						selectable: 1,
            ...
            $('.jquery-modal').removeClass('blocker');
				...
				function getMaterialColumnsConfig() {
					var columnsConfig =  {
						columns: [
							{ title: i18next.t("COAT ID"), field: "MATERIAL_CODE", headerFilter: true},
							{ title: i18next.t("COAT"), field: "MATERIAL_NAME_SHORT", headerFilter: true}
						],
						placeholder: i18next.t("No Data Available")
					}
					return columnsConfig;
				...
				let productGrades; let baseURL;
				let productTable;
				function setupProductTabulator(rowData, tableData) {
					let tableConfig = getProductColumnsConfig();
					productTable = new Tabulator("#iFrame-product-table", {
						data: [],
						selectable: 1,
						pagination: true,
						paginationSize: 10,
					...
					productTable.on("rowSelected", function() {						
						var selectedData = productTable.getSelectedData();
						const index = tableData.findIndex(x => JSON.stringify(x) === JSON.stringify(rowData));
						tableData.splice(index, 1);
						rowData.COAT_ID = selectedData[0].PRODUCT_CODE;
						rowData.COATING = selectedData[0].PRODUCT_NAME_SHORT;						
						rowData.TARGET_VALUE = selectedData[0].GRAMMAGE;
						tableData.push(rowData);
            ...
            $('.jquery-modal').removeClass('blocker');
				...
				function getProductColumnsConfig() {
					var columnsConfig =  {
						columns: [
							{ title: i18next.t("PRODUCT CODE"), field: "PRODUCT_CODE", headerFilter: true},
							{ title: i18next.t("PRODUCT NAME"), field: "PRODUCT_NAME_SHORT", headerFilter: true},
							{ title: i18next.t("GSM"), field: "GRAMMAGE", headerFilter: true},
							{ title: i18next.t("GRADE"), field: "GRADE_CODE", headerFilter: true}
						],
						placeholder: i18next.t("No Data Available")
					}
					return columnsConfig;
				...
				function getProductCodes() {
					const fromGSM = document.getElementById("fromGSM").value;
					const toGSM = document.getElementById("toGSM").value == ""? '9999' : document.getElementById("toGSM").value;
					const grade = document.getElementById("grade").value == undefined? 'NA' : document.getElementById("grade").value;
					fuctionType = 'BA';
					
					$.ajax({
						url: baseURL + "/product/getfiltered",
            type: "POST",
						headers : {'Authorization': getCookie("tokensso") },
						data: JSON.stringify({ "FROM_NUM": Number(fromGSM), "TO_NUM": Number(toGSM), "GRADE": grade, "FUNCTION_TYPE": "BP"}),
            success: function (data) {							
							productTable.replaceData(data);
            }, 
            error: function (error) { }
				...
				function loadSelectGrade() {
					$.ajax({
						url: baseURL + "/product/getproductgrades",
            type: "GET",
						headers : {'Authorization': getCookie("tokensso") },
						success: function (data) {
        	...
        	if (parts.length === 2) return parts.pop().split(';').shift();
    			...
    			<input type="number" class="textinput" id="mainMaterialBoneDryInput" disabled>
				</div>
				...
				<script>
					...
					let recipeFCTOption ; let recipeItemType;let recipeClass;
							...
							table = new Tabulator("#iFrame-table", {
								...
								rowFormatter: function(row) {
									...
									row.getCells().forEach((cell) => {
										...
										const field = ["PROPERTY_CODE","COMMAND_TYPE","UNIT","COAT_ID","COATING","CALC_MINIMUM","CALC_MAXIMUM"];
										if(field.includes(fieldName)) {
												cell.getElement().style.backgroundColor="#dbdbdb";
											} else if(fieldName == 'TARGET_VALUE') {
												if((data.PROPERTY_CODE == '' || data.PROPERTY_CODE == null || data.RECIPE_FUNCTION == '' || data.RECIPE_FUNCTION == null || data.RECIPE_FUNCTION == 'BP')){
													cell.getElement().style.backgroundColor="#dbdbdb";
												} else {
													cell.getElement().style.backgroundColor="";
												}
											} else if(fieldName == 'IN_VAL') {
												if((data.TARGET_VALUE == "" || data.TARGET_VALUE == null)) {
													cell.getElement().style.backgroundColor="#dbdbdb";
												} else {
													cell.getElement().style.backgroundColor="";
												}
											} else if(fieldName == 'INPUT_MINIMUM') {
												if(data.TARGET_VALUE == "" || data.TARGET_VALUE == null || data.IN_VAL == "" || data.IN_VAL == null) {
													cell.getElement().style.backgroundColor="#dbdbdb";
												} else {
													cell.getElement().style.backgroundColor="";
												}
					...
					function getColumnsConfig() {
						var columnsConfig =  {
							columns: [
								{ title: i18next.t("LOSS"), field: "LOSS_GROUP_NUMBER", editor:"input", tooltip:true,
								...
								{ title: i18next.t("LIMIT SHO"), field: "COMMAND_TYPE", tooltip:true },
								...
								{ title: i18next.t("TARGET"), field: "TARGET_VALUE", editor:"input", tooltip:true,visible: columnConfiguration.includes('%BDRY') ? true: false,
									editable: function(cell) {
										let rowData = cell.getRow().getData();
										return rowData.PROPERTY_CODE == '' || rowData.PROPERTY_CODE == null || rowData.RECIPE_FUNCTION == '' || rowData.RECIPE_FUNCTION == null || rowData.RECIPE_FUNCTION == 'BP'? false : true;
									},
									cellEdited: function(cell) {
										onUpdateTable(cell);
										let tableData = table.getData();
										...
										for(let i = 0; i < tableData.length; i++) {
											if(tableData[i].RECIPE_FUNCTION==='AB'){
												totalTargetValue = totalTargetValue + Number(tableData[i].TARGET_VALUE);	
											}
										}
										mainMaterialBoneDry = totalTargetValue;
										onTargetValueChange();
										document.getElementById("mainMaterialBoneDryInput").value = Number(mainMaterialBoneDry).toFixed(2);
										message.messageEmitterName = 'CALCULATED_VALUE';
										message.Message = mainMaterialBoneDry;
										window.parent.postMessage(message, '*');
									}
								},								
								...
								{ title: i18next.t("SEARCH"), align:"center", formatter: cellSearch,
									cellClick: function(e, cell) {
										...
										if(rowData.RECIPE_FUNCTION == 'AB' || rowData.RECIPE_FUNCTION == 'PE') {
											setupMaterialTabulator(cell.getRow().getData(), table.getData());
											$('#material-div').modal('show');
										} else if(rowData.RECIPE_FUNCTION == 'BP') {
											document.getElementById("fromGSM").value = "1";
											document.getElementById("toGSM").value = "";
											document.getElementById("grade").selectedIndex = 0;
											setupProductTabulator(cell.getRow().getData(), table.getData());											
											$('#product-div').modal('show');											
										}
								...
								{ title: i18next.t("COAT ID"), field: "COAT_ID", tooltip:true },
								{ title: i18next.t("COAT"), field: "COATING", tooltip:true },								
								{ title: i18next.t("LIMIT TYPE"), field: "IN_VAL", editor:"list", tooltip:true,
									editorParams: {values: {"A" : "Absolute", "P" : "Percentage", "R" : "Relative"}},
									editable: function(cell) {
										let rowData = cell.getRow().getData();
										return rowData.TARGET_VALUE == "" || rowData.TARGET_VALUE == null? false : true;
									},
									cellEdited: function(cell) {
										onUpdateTable(cell);
										onTargetValueChange();
								...
								{ title: i18next.t("MIN"), field: "INPUT_MINIMUM", editor:"input", tooltip:true,
									editable: function(cell) {
										let rowData = cell.getRow().getData();
										return rowData.TARGET_VALUE == "" || rowData.TARGET_VALUE == null || rowData.IN_VAL == "" || rowData.IN_VAL == null? false : true;
									},
									cellEdited: function(cell) {
										onUpdateTable(cell);										
										let tableData = table.getData();
										for(let i = 0; i < tableData.length; i++) {
											if(tableData[i].IN_VAL == 'A') {
												...
												tableData[i].CALC_MINIMUM = Number(tableData[i].TARGET_VALUE) - Number(tableData[i].INPUT_MINIMUM);
											} else if(tableData[i].IN_VAL == 'P') {
												var per = (Number(tableData[i].TARGET_VALUE) * Number(tableData[i].INPUT_MINIMUM)) / 100;
												tableData[i].CALC_MINIMUM = Number(tableData[i].TARGET_VALUE) - Number(per);
											}											
										}
									...
									cellEdited: function(cell) {
										...
										for(let i = 0; i < tableData.length; i++) {
											if(tableData[i].IN_VAL == 'A') {
												...
												tableData[i].CALC_MAXIMUM = Number(tableData[i].TARGET_VALUE) + Number(tableData[i].INPUT_MAXIMUM);
											} else if(tableData[i].IN_VAL == 'P') {
												var per = (Number(tableData[i].TARGET_VALUE) * Number(tableData[i].INPUT_MAXIMUM)) / 100;
												tableData[i].CALC_MAXIMUM = Number(tableData[i].TARGET_VALUE) + Number(per);
											}
										}
										if(tableData.length > 0) {
											table.replaceData(tableData);
								...
								{ title: i18next.t("RECIPE ITEM TYPE"), field: "RECIPE_ITEM_TYPE",visible: false, editor:"input", tooltip:true,
									cellEdited: function(cell) {
										onUpdateTable(cell);
								...
								{ title: i18next.t("DELETE"), field: "customColumn2", align: "center", editor: true, formatter: "tickCross",
									cellEdited: function(cell) {
										onUpdateTable(cell);
									}
								},
					...
					function chkBdryClick() {
					  const chk = document.getElementById("chkbdry");
							if(chk.checked) {
									table.showColumn('TARGET_VALUE');
							 }else {
									table.hideColumn('TARGET_VALUE');
							 }
					};
      			...
      			table.addRow(newRow, false,rowCount);
					...
					function onTargetValueChange() {		
						...
						for(let i = 0; i < tableData.length; i++) {
							if(tableData[i].IN_VAL == 'A') {
								...
								if(tableData[i].INPUT_MAXIMUM != null && tableData[i].INPUT_MAXIMUM != "") {
									var per1 = (Number(tableData[i].TARGET_VALUE) * Number(tableData[i].INPUT_MAXIMUM)) / 100;
									tableData[i].CALC_MAXIMUM = Number(tableData[i].TARGET_VALUE) + Number(per1);
								}
							}
						}
						if(tableData.length > 0) {
					...
					function updatei18nextText() {
						document.getElementById('totalLabel').innerText = i18next.t("Total");
						...
						document.getElementById('GSMLabel').innerText = i18next.t("GSM");
						document.getElementById('GradeLabel').innerText = i18next.t("Grade");
						document.getElementById('searchButton').innerText = i18next.t("Search");
					...
					$('#recipeButton').prop('disabled', false);
						if(event.data.message == "CREATE") {
							...
							loadSelectGrade();
							document.getElementById("mainMaterialBoneDryInput").value = Number(mainMaterialBoneDry).toFixed(2);
							apiURL = message.BASEURL + "/recipeitem/getfiltered"
							...
							let totalTargetValue = 0;
							for(let i = 0; i < tableData.length; i++) {
								totalTargetValue = totalTargetValue + Number(tableData[i].ACTUAL_VALUE);
							}
							mainMaterialBoneDry = totalTargetValue;
							onTargetValueChange();
