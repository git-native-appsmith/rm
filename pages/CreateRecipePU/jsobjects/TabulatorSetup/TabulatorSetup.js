...
/*# sourceMappingURL=tabulator.min.css.map */
			...
			<div id="pu-popup-div" class="modal" style="position:absolute; top: 5%; left: 5%; max-width: 100%!important">
				<div id="iFrame-pu-popup-table"></div>
			</div>			
			<script>
				let puPopupTable; let OPRTYPE;
				function setupPUPopupTabulator(rowData, tableData) {
					let tableConfig = getPUPopupColumnsConfig(rowData);
					let puPopupAPIBody = {machineGroup: resourceSelect , OPRTYPE: OPRTYPE};
					puPopupTable = new Tabulator("#iFrame-pu-popup-table", {
						ajaxURL: baseURL + "/getmaterialproperty",
						ajaxContentType: {
							body: function(url, config, params) {
								return JSON.stringify(puPopupAPIBody);
							},
						},
						ajaxConfig: {
							method: "POST",
					...
					puPopupTable.on("rowSelected", function() {						
						var selectedData = puPopupTable.getSelectedData();
						...
						$('#pu-popup-div').modal().hide();						
            $('.jquery-modal').removeClass('blocker');
						updateSearchViewButtonText();
					});
				};
				function getPUPopupColumnsConfig(rowData) {
					var columnsConfig =  {
						columns: [
        	...
        	if (parts.length === 2) return parts.pop().split(';').shift();
								...
								row.getCells().forEach((cell) => {
									...
									if(field.includes(fieldName)) {
									  	cell.getElement().style.backgroundColor="#dbdbdb";
											...
											if(!disableVolumeField(row.getData())) {
												cell.getElement().style.backgroundColor="#dbdbdb";
											} else {
											  cell.getElement().style.backgroundColor="";
											}
							...
							columns: [		
								...
								{ title: i18next.t("SEARCH"), align:"center", formatter: cellSearch,
									cellClick: function(e, cell) {
										...
										setupPUPopupTabulator(cell.getRow().getData(), table.getData());
										$('#pu-popup-div').modal('show');
                ...
                { title: i18next.t("MIN"), field: "INPUT_MINIMUM", editor:"input", tooltip:true,
								...
								{ title: i18next.t("WEIGHT"), field: "TARGET_VALUE_WEIGHT", editor:"input", tooltip:true,visible: columnConfiguration.includes('WEIGHT') ? true: false,editable: function(cell) { return disableWeightField(cell.getRow().getData());},
									cellEdited: function(cell) {
										onWeightChange();
										onTargetValueChange();
									}
								},
								{ title: i18next.t("VOLUME"), field: "VOLUME", editor:"input", tooltip:true,editable: function(cell) { return disableVolumeField(cell.getRow().getData());},visible: columnConfiguration.includes('VOLUME') ? true: false},
          ...
          function onUpdateTable(cell) {
					...
					function disableVolumeField(currentRow) {						
						let isEditable = true;
						if( currentRow.RECIPE_FUNCTION == '' || currentRow.RECIPE_FUNCTION == undefined) {
							return isEditable = false;
						} else {
					...
					function onValueChange() {
						...
						if(tableData.length > 0) {
							...
							message.Message = {"mainMaterialBoneDry" : sv_mainMtrlBoneDry, "SPWT" : 0, "GBT": 0, "PER": totalper, "totalWeight": 0, "totalVolumn": 0, "filler": 0, "TDC": 0, "DC": 0, "errorMessage": ''};
							window.parent.postMessage(message, '*');
						}
					}
