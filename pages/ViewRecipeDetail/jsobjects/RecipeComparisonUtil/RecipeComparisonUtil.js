export default {
	...
	OpenComparisionPopup: () => {
		const recipeStatus = GetFilteredRecipe.data.RECIPE_STATUS;
		if((recipeStatus == "A" || recipeStatus == "a") && GetFilteredRecipe.data.REVISION_NO > 1) {
			if(GetComparison.data == null || GetComparison.data == undefined) {
				GetComparison.run();
			}
			showModal("RecipeViewComparisonModal");
		} else if((recipeStatus == "n" || recipeStatus== "N" || recipeStatus == "P" || recipeStatus == "p") && GetFilteredRecipe.data.REVISION_NO > 1) {
			if(GetComparison.data == null || GetComparison.data == undefined) {
				GetComparison.run();
			}
			showModal("RecipeViewComparisonModal");
