export default {
	onPageLoad: async () => {
		...
		if(appsmith.URL.queryParams.RecipeAction == "Create") {
			postWindowMessage({
				message: "CREATE",
				data : {"BASEURL": baseURL, "RESOURCE_SELECT": resourceSelect, "TARGET_DRY_CONTENT": 0, "MAIN_MATERIAL_BONE_DRY": 1000, "SPEC_WEIGHT": specWeight, "TOTAL_WEIGHT": totalWeight, "FILLER": filler, "GBT": gbt, "CALCULATED_DRY_CONTENT": calculatedDryContent, "MAIN_MATERIAL": mainMaterial, "MAX_VOLUME": maxVolume, "TOTAL_VOLUME": totalVolume,"RECIPE_TYPE": recipeType,"WEIGHT_UNIT": weightUnit,"VOLUME_UNIT": volumeUnit,"RECIPE_CLASS": appsmith.store.recipeClassCode, "SV_TBL_Limit": appsmith.store.SV_TBL_Limit, "MACHINE_GROUP_NAME": appsmith.store.ResourceGroupDropDownName, "RECIPE_TYPE_NAME": appsmith.store.RecipeTypeDropDownName, "RECIPE_NAME": appsmith.store.InterMaterialName, "CALC_FIELD": appsmith.store.Calc_dryContentSpecWeightLimitField}
			}, 'Iframe1', '*');
		}
