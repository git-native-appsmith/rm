...
/*# sourceMappingURL=tabulator.min.css.map */
				...
				function setupPropertyMaterialTabulator(rowData, tableData) {
					...
					propertyMaterialTable = new Tabulator("#iFrame-property-material-table", {						
						...
						ajaxResponse: function (url, params, response) {							
							return {
								data: rowData.RECIPE_ITEM_TYPE === 'DOS'? response.filter(x => x.MATERIAL_CODE != interMediateMaterial) : response,
							};
						},
						selectable: 1,
						maxHeight:"500px",
					...
					propertyMaterialTable.on("rowSelected", function() {						
						...
						rowData.BALE_WEIGHT = selectedData[0].WEIGHT;
            ...
            $('.jquery-modal').removeClass('blocker');
				...
				function getPropertyMaterialColumnsConfig(rowData) {
					var columnsConfig =  {
						columns: [
							{ title: rowData.RECIPE_ITEM_TYPE === 'DOS'? i18next.t("MATERIAL CODE") : i18next.t("PROPERTY CODE"), field: "MATERIAL_CODE", headerFilter: true, tooltip: true, headerTooltip: true, minWidth: '160px'},
							{ title: rowData.RECIPE_ITEM_TYPE === 'DOS'? i18next.t("NAME") : i18next.t("NAME"), field: "MATERIAL_NAME_SHORT", headerFilter: true, tooltip: true, headerTooltip: true, minWidth: '160px'},
							{ title: i18next.t("NID"), field: "NID", visible: rowData.RECIPE_ITEM_TYPE === 'DOS'? true : false},
							{ title: i18next.t("DC"), field: "CONCENTRATION", visible: rowData.RECIPE_ITEM_TYPE === 'DOS'? true : false},
							{ title: i18next.t("DENSITY"), field: "SPECIFIC_WEIGHT", visible: rowData.RECIPE_ITEM_TYPE === 'DOS'? true : false, headerTooltip: true},
							{ title: i18next.t("FILLER"), field: "FILLER", visible: rowData.RECIPE_ITEM_TYPE === 'DOS'? true : false},
							{ title: i18next.t("UNIT"), field: "UNIT"},
							{ title: i18next.t("WEIGHT"), field: "WEIGHT", visible: false},
							{ title: i18next.t("NO. OF BALES"), field: "NO_OF_BALES", visible: rowData.RECIPE_ITEM_TYPE === 'DOS'? true : false, headerTooltip: true},
						],
        	...
        	if (parts.length === 2) return parts.pop().split(';').shift();
				...
				<div id='columnVisibilityDiv' class='calculationDiv' hidden>
				  <label id="lblcolumnvisibility" for="bdry">Column visibility</label>
					...
					<input style="margin-left: 15px" type="checkbox" id="chkbales" name="BALES"  onclick="chkBalesClick()"/>
					<label id="lblbales" for="bdry">BALES</label>
				</div>
				...
				<script>
					...
					let weightUnit; let volumeUnit; let recipeClass;let recipeFCTOption ; let recipeItemType;
								...
								row.getCells().forEach((cell) => {
									...
									if(field.includes(fieldName)) {
									  	cell.getElement().style.backgroundColor="#dbdbdb";
											...
											if(!disableValueField(data)) {
												cell.getElement().style.backgroundColor="#dbdbdb";
											} else {
											  cell.getElement().style.backgroundColor="";
											}
										}else if(fieldName == 'TARGET_VALUE_WEIGHT') {
											if(!disableWeightField(data)) {
												cell.getElement().style.backgroundColor="#dbdbdb";
											} else {
											  cell.getElement().style.backgroundColor="";
											}
										}else if(fieldName == 'VOLUME') {
											if(!disableVolumnField(data)) {
												cell.getElement().style.backgroundColor="#dbdbdb";
											} else {
											  cell.getElement().style.backgroundColor="";
											}
										}else if(fieldName == 'BALES') {
											if(!disableBaleField(data)) {
												cell.getElement().style.backgroundColor="#dbdbdb";
											} else {
											  cell.getElement().style.backgroundColor="";
											}
					...
					function getColumnsConfig() {
						var columnsConfig =  {
							columns: [		
								{ title: i18next.t("GRP"), field: "GROUP_NUMBER", editor:"input", tooltip:true,headerTooltip:true },
								{ title: i18next.t("SEQ"), field: "SEQ_NO", editor:"input", tooltip:true,headerTooltip:true,
									cellEdited: function(cell) {
								...
								{ title: i18next.t("CMD"), field: "RECIPE_ITEM_TYPE", editor:"list", tooltip:true,headerTooltip:true,
								...
								{ title: i18next.t("SEARCH"), align:"center", formatter: cellSearch,
									cellClick: function(e, cell) {
										OPRTYPE = cell.getRow().getData().RECIPE_ITEM_TYPE === 'DOS'? 1 : 2;
								...
								{ title: i18next.t("MATERIAL/PROP.C"), field: "MATERIAL_COMMAND_CODE", tooltip:true,headerTooltip:true },
								{ title: i18next.t("NAME"), field: "MATERIAL_COMMAND", tooltip:true ,headerTooltip:true},
								{ title: i18next.t("NID"), field: "NID", tooltip:true,headerTooltip:true,visible: columnConfiguration.includes('NID') ? true: false },
								{ title: i18next.t("FCT"), field: "RECIPE_FUNCTION", editor:"list", tooltip:true,headerTooltip:true,
								...
								{ title: i18next.t("TARGET"), field: "VALUE_PERCENTAGE", editor:"input", tooltip:true,headerTooltip:true,visible: columnConfiguration.includes('%BDRY') ? true: true,headerTooltip:true,
									editable: function(cell) { return disableValueField(cell.getRow().getData());},
									cellEdited: function(cell) {
										onTotalBoneDryPulp();
									},
								},
								{ title: i18next.t("UNIT"), field: "UNIT", tooltip:true ,headerTooltip:true},
								{ title: i18next.t("WEIGHT"), field: "TARGET_VALUE_WEIGHT", editor:"input", tooltip:true,headerTooltip:true,visible: columnConfiguration.includes('WEIGHT') ? true: false,
									editable: function(cell) { return disableWeightField(cell.getRow().getData());},
									cellEdited: function(cell) {
										onTotalBoneDryPulp();
									},
								},
								{ title: i18next.t("VOLUME"), field: "VOLUME", editor:"input", tooltip:true,headerTooltip:true,visible: columnConfiguration.includes('VOLUME') ? true: false,
									editable: function(cell) { return disableVolumnField(cell.getRow().getData());},
									cellEdited: function(cell) {
										onTotalBoneDryPulp();
									},
								},
								{ title: i18next.t("BALES"), field: "BALES", editor: "input", tooltip:true,headerTooltip:true,visible: columnConfiguration.includes('BALES') ? true: false,
									editable: function(cell) {return disableBaleField(cell.getRow().getData()); },
									cellEdited: function(cell) {
										var tableData = table.getData();
										let totalBoneDry = 0; let totalVolume = 0; let totalWeight = 0; let totalper = 0; let totFiller = 0; let bale_per = 0; 
										let WT = 1; let VL = 1;
										for(let i = 0; i < tableData.length; i++) {
											if(tableData[i].RECIPE_ITEM_TYPE != 'CMD'){
												...
												let bales = tableData[i].BALES == '' ? 0 : Math.round(tableData[i].BALES);
												let baleWeight = tableData[i].BALE_WEIGHT == '' ? 0 : Number(tableData[i].BALE_WEIGHT);
												if(tableData[i].RECIPE_FUNCTION != 'CAL') {
													if(tableData[i].RECIPE_FUNCTION == 'FIX' && baleWeight > 0) {
													...
													} else if(tableData[i].RECIPE_FUNCTION == 'FIX' && baleWeight == 0) {
										...
										let x = 100 - bale_per;
										let main_totalBoneDry = 0;
										if(x>0){
											main_totalBoneDry = totalBoneDry * 100 / x;
										}
										...
										if(isNaN(sv_mainMtrlBoneDry)||sv_mainMtrlBoneDry==0) {
											mainMaterialBoneDry = 0;
											sv_mainMtrlBoneDry=''
										}
										if(isNaN(sv_dc)||sv_dc==0) {
											sv_dc = '';
										}
										totalBoneDry = 0;
										for(let i=0; i <tableData.length;i++)
										{
											if(tableData[i].RECIPE_ITEM_TYPE != 'CMD') {
												...
												let bales = tableData[i].BALES == '' ? 0 : Math.round(tableData[i].BALES);
												let baleWeight = tableData[i].BALE_WEIGHT == '' ? 0 : tableData[i].BALE_WEIGHT;
												if(tableData[i].RECIPE_FUNCTION != 'CAL') {
								...
								{ title: i18next.t("DC"), field: "DC_CONCENTRATION", tooltip:true,headerTooltip:true },								
								{ title: i18next.t("DENSITY"), field: "SPECIFIC_WEIGHT", tooltip:true,headerTooltip:true },
								{ title: i18next.t("FILLER"), field: "FILLER", tooltip:true ,headerTooltip:true},
								{ title: i18next.t("DELETE"), field: "customColumn2", align: "center", editor: true, formatter: "tickCross",headerTooltip:true },
								{ title: i18next.t("VIEW"), align:"center", formatter: cellView,headerTooltip:true, visible: true,
									cellClick: function(e, cell) {
      			...
      			table.addRow({}, false, rowCount);
					...
					function disableBaleField(currentRow) {
						let isEditable = true;
						if(currentRow.RECIPE_ITEM_TYPE == 'CMD') {
							return isEditable = false;
						} else if(currentRow.RECIPE_ITEM_TYPE == 'DOS') {
							if(currentRow.RECIPE_FUNCTION=='CAL') {
								isEditable = false;
							}
							if(currentRow.BALE_WEIGHT == 0) {
								isEditable = false;
							}
						}
						return isEditable;
						...
						if(tableData.length > 0) {
							...
							for(let i=0; i <tableData.length;i++)
							{
								if(tableData[i].RECIPE_ITEM_TYPE != 'CMD'){
									...
									filler = tableData[i].FILLER==''?0:tableData[i].FILLER;
									let bales =0;
									let baleWeight = tableData[i].LOSS_GROUP_NUMBER==''?0:tableData[i].LOSS_GROUP_NUMBER
									if(tableData[i].RECIPE_FUNCTION != 'CAL'){
											weight=tableData[i].TARGET_VALUE_WEIGHT==''?0:Number(tableData[i].TARGET_VALUE_WEIGHT);
											volume=tableData[i].VOLUME==''?0:Number(tableData[i].VOLUME);
					...
					function updatei18nextText() {
						...
						document.getElementById('lblbales').innerText = i18next.t("BALES");
					...
					function chkBalesClick() {
					  const chk = document.getElementById("chkbales");
							if(chk.checked) {
									table.showColumn('BALES');
							 }else {
									table.hideColumn('BALES');
							 }
					};
					function checkedCheckBoxconfig() {
					 if(columnConfiguration.includes('%BDRY')) {
							...
							if(columnConfiguration.includes('BALES')) {
								document.querySelector("#chkbales").checked = true
							}
					};
					...
					$('#recipeButton').prop('disabled', false);
						if(event.data.message == "CREATE") {
							...
							if(targetDryContent == '') targetDryContent = 0;
							if(mainMaterialBoneDry == '') mainMaterialBoneDry = 0;
