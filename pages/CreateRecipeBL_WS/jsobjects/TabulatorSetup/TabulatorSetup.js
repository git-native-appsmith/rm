...
/*# sourceMappingURL=tabulator.min.css.map */
            ...
            $('.jquery-modal').removeClass('blocker');
				...
				function getPropertyColumnsConfig() {
					var columnsConfig =  {
						columns: [
							...
							{ title: i18next.t("TARGET WORK PLACE"), field: "TARGET_WORK_PLACE"},
				...
				let materailTable;let OPRTYPE;
				function setupMaterialTabulator(rowData, tableData) {
					let tableConfig = getMaterialColumnsConfig(rowData);
					materailTable = new Tabulator("#iFrame-material-table", {
						ajaxURL: baseURL + "/getMaterialProperty",
						ajaxContentType: {
							body: function(url, config, params) {
								return JSON.stringify({ "machineGroup": resourceSelect, "OPRTYPE": OPRTYPE});
							},
						},
						ajaxConfig: {
							method:"POST",
            ...
            $('.jquery-modal').removeClass('blocker');
				...
				function getMaterialColumnsConfig(rowData) {
					var columnsConfig =  {
						columns: [
							{ title: i18next.t(getHeaderCode(rowData.RECIPE_ITEM_TYPE)), field: "MATERIAL_CODE", headerFilter: true},
							{ title: i18next.t("NAME"), field: "MATERIAL_NAME_SHORT", headerFilter: true}
						],
						placeholder: i18next.t("No Data available")
					}
        	...
        	if (parts.length === 2) return parts.pop().split(';').shift();
				...
				<script>
					...
					let apiURL; let apiBody; let baseURL;let recipeClass;let recipeFCTOption ; let recipeItemType;
						...
						if(apiURL != undefined) {
							...
							table = new Tabulator("#iFrame-table", {
								...
								row.getCells().forEach((cell) => {
									...
									const field = ["PROPERTY_CODE","COMMAND_TYPE","UNIT","COAT_ID","COATING","CALC_MINIMUM","CALC_MAXIMUM","TARGET_WORK_PLACE","SECTION"];
							...
							columns: [								
								...
								{ title: i18next.t("TARGET"), field: "TARGET_VALUE", editor:"input", tooltip:true,visible: columnConfiguration.includes('%BDRY') ? true: false,
									editable: function(cell) {
										let rowData = cell.getRow().getData();
										return rowData.PROPERTY_CODE == '' || rowData.PROPERTY_CODE == null || rowData.TARGET_V == '' || rowData.TARGET_V == null? true : true;
									},
									cellEdited: function(cell) {
										onUpdateTable(cell);
										onTargetValueChange();
								...
								{ title: i18next.t("MTRL CODE"), field: "COAT_ID", tooltip:true },
								...
								{ title: i18next.t("SECTION"), field: "SECTION", tooltip:true },
      			...
      			table.addRow(newRow, false,rowCount);
					...
					$('#recipeButton').prop('disabled', false);
						if(event.data.message == "CREATE") {
							...
							document.getElementsByClassName("loader")[0].style.display = "none";
							table.replaceData(event.data.data);
						} else if(event.data.message == "closeLoader") {
						  document.getElementsByClassName("loader")[0].style.display = "none";
						}					
					});
				</script>
