export default {
	...
	OpenComparisionPopup: () => {
		console.log('Pop');
		if((RecipeTable.selectedRow.RECIPE_STATUS == "A" || RecipeTable.selectedRow.RECIPE_STATUS == "a") && GetFilteredRecipeView.data.REVISION_NO > 1) {
			if(GetComparison.data == null || GetComparison.data == undefined) {
				GetComparison.run();
			}
			showModal("RecipeViewComparisionModal");
		} else if((RecipeTable.selectedRow.RECIPE_STATUS == "n" || RecipeTable.selectedRow.RECIPE_STATUS == "N" || RecipeTable.selectedRow.RECIPE_STATUS == "P" || RecipeTable.selectedRow.RECIPE_STATUS == "p") && GetFilteredRecipeView.data.REVISION_NO > 1) {
			if(GetComparison.data == null || GetComparison.data == undefined) {
				GetComparison.run();
			}
			showModal("RecipeViewComparisionModal");
