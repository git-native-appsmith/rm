export default {
	...
	showErrorModalNew: async () => {
		if(!appsmith.store.IsCreate_RecipeLoadFirstTime) return false;
		let mVolume = Number(GetProductionRecipeForView.data.MAX_VOLUME);
	...
	OnSaveClickValidation:() => {
		...
		if(appsmith.store.RecipeClassDropDown == "PD" || appsmith.store.RecipeClassDropDown == "RM" || appsmith.store.RecipeClassDropDown == "CO") {
			if(isNaN(Number(GetProductionRecipeForView.data.MAX_VOLUME)) || Number(GetProductionRecipeForView.data.MAX_VOLUME) < 0) {
			...
			if(isNaN(Number(appsmith.store.calculatedField.sv_totalVolume)) || Number(appsmith.store.calculatedField.sv_dc) < 0) {
		...
		if(classification == 'ST') {
			...
			if(DatePicker1.selectedDate == ""){
				validationMsg = validationMsg + count +".Please Select date. ";
					count++;
			}
		}
