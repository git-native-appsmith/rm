...
<body>
  <div>
	...
	<script>
		...
		let MODIF_NUMBER;	let RecipeSetCategory;	
		function getColumnsConfig() {
			...
			[
				...
				disabled: function(cell) 
						  {
								var rowData = cell.getRow().getData();
								if(MODIF_NUMBER == "" || MODIF_NUMBER == undefined) {
									return true;
								} else {
									if(String(rowData.RECIPE_STATUS) == 'N' || String(rowData.RECIPE_STATUS) == 'n') {
										if(Number(rowData.MODIF_NUMBER) == Number(MODIF_NUMBER)) {
											return false;
										} else {
											return true;
										}
										...
										if(RecipeSetCategory == 'Standard') {
											if(rowData.IS_RECIPE_EXIST_WITH_SAME_REVISION) {
												return true;
											} else {
												return false;
											}
				...
				action:function(e, cell)
							{
								message.messageEmitterName = 'DELETE';
								message.Message = {"rowData" : cell.getRow().getData()};
								window.parent.postMessage(message, '*');
							},
				disabled: function(cell) 
				...
				{
				label: i18next.t("ASSOCIATE RECIPESET"),
				action:function(e, cell)
							{
								message.messageEmitterName = 'ASSOCIATE';
								message.Message = {"rowData" : cell.getRow().getData()};
								window.parent.postMessage(message, '*');
							},
				disabled: function(cell) 
						  {
								if(MODIF_NUMBER==='') return true;
								else return false;
							}
				},
				{
				label: i18next.t("RECIPE INCONSISTENCIES"),
				action:function(e, cell)
							{
								message.messageEmitterName = 'INCONSISTENCIES';
								message.Message = {"rowData" : cell.getRow().getData()};
								window.parent.postMessage(message, '*');
							},
				disabled: function(cell) 
						  {
								var rowData = cell.getRow().getData();
								if(rowData.IS_VALID === 0) return true;
								else return false;
							}
				}
			];
			var columnsConfig =  {
				columns: [
					{ title: i18next.t("RECIPE NUMBER"), field: "RECIPE_NUMBER",visible:false},
					...
					{ title: i18next.t("MODIF NO"), field: "MODIF_NUMBER"},
					{ title: i18next.t("MODIFIED DATE"), field: "RECIPE_UPTIME"},
					{ title: i18next.t("USER"), field: "RECIPE_UPUSER" ,visible:true},
					{ title: i18next.t("CLASSIFICATION"), field: "CLASSIFICATION",visible:false},
					{ title: i18next.t("RECIPE TYPE NUMBER"), field: "RECIPE_TYPE_NUMBER" ,visible:false},
					{ title: i18next.t("IS RECIPE EXIST WITH SAME REVISION"), field: "IS_RECIPE_EXIST_WITH_SAME_REVISION" ,visible:false}, 
					{ title: i18next.t("RECIPE CLASS CODE"), field: "RECIPE_CLASS_CODE" ,visible:false},
					{ title: i18next.t("MACHINE GROUP"), field: "MACHINE_GROUP" },
					{ title: i18next.t("RECIPE TYPE NAME"), field: "RECIPE_TYPE_NAME"},
					{ title: i18next.t("IS VALID"), field: "IS_VALID" ,visible:false },
					{ title: i18next.t("STATION PLACE"), field: "STATION_PLACE" ,visible:false},
					{ title: i18next.t("WORKPLACE"), field: "PLACE_NAME_SHORT"},
			...
			table = new Tabulator("#Recipe-table", {
								...
								ajaxResponse: !tableConfig?.isEmpty && function (url, params, response) {
									...
									for(let i=0; i <response.length;i++){
										if(MODIF_NUMBER == "" || MODIF_NUMBER == undefined) {
											response[i].RECIPE_STATUS= response[i].RECIPE_STATUS.toLowerCase();		
										} else {
											if(String(response[i].RECIPE_STATUS) == 'N' ) {
												if(Number(response[i].MODIF_NUMBER) == Number(MODIF_NUMBER)) {
													response[i].RECIPE_STATUS= response[i].RECIPE_STATUS.toUpperCase();		
												} else {
													response[i].RECIPE_STATUS= response[i].RECIPE_STATUS.toLowerCase();		
												}
												...
												if(RecipeSetCategory == 'Standard') {
													if(response[i].IS_RECIPE_EXIST_WITH_SAME_REVISION) {
														response[i].RECIPE_STATUS= response[i].RECIPE_STATUS.toLowerCase();		
													} else {
														response[i].RECIPE_STATUS= response[i].RECIPE_STATUS.toUpperCase();		
													}
	...
	function setupTableOnCheckBox() {
		...
		window.addEventListener("message", (event) => {
			...
			if (event.data?.message === "View") {
				...
				MODIF_NUMBER=message.MODIF_NUMBER;
				RecipeSetCategory=message.RecipeSetCategory;
				apiURL = message.BASE_URL+"/recipe/getfiltered";
				apiBody = {
							"recipeSetName":message.recipeSetName,
							"machineGroup": message.machineGroup,
							"recipeClass": message.recipeClass,
							"recipeTypeNumber": message.recipeTypeNumber,
							"productGroupNumber": 0,
							"productCode": message.productCode,
							"intermediate_Mat_Code": message.intermediate_Mat_Code,
							"property_Code": message.property_Code,
							"recipenumber": message.recipenumber
						};			
				//setupTable();
