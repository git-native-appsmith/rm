...
/*# sourceMappingURL=tabulator.min.css.map */
				...
				function getProductColumnsConfig() {
					var columnsConfig =  {
						columns: [
							...
							{ title: i18next.t("NAME"), field: "PRODUCT_NAME_SHORT", headerFilter: true},
        	...
        	if (parts.length === 2) return parts.pop().split(';').shift();
    			...
    			<input type="number" class="textinput" disabled id="mainMaterialBoneDryInput">
				</div>
				...
				<script>
					...
					let apiURL; let apiBody;let recipeFCTOption ;let recipeClass;
					...
					function getColumnsConfig() {
						var columnsConfig =  {
							columns: [
								...
								{ title: i18next.t("LIMIT S"), field: "COMMAND_TYPE", tooltip:true },
									...
									cellEdited: function(cell) {
										...
										for(let i = 0; i < tableData.length; i++) {
											if(tableData[i].IN_VAL == 'A') {
												...
												var per = (Number(tableData[i].ACTUAL_VALUE) * Number(tableData[i].INPUT_MAXIMUM)) / 100;
												tableData[i].CALC_MAXIMUM = Number(tableData[i].ACTUAL_VALUE) + Number(per);
											}
										}
										if(tableData.length > 0) {
											table.replaceData(tableData);
								...
								{ title: i18next.t("RECIPE ITEM TYPE"), field: "RECIPE_ITEM_TYPE", editor:"input", visible: false,tooltip:true,
									cellEdited: function(cell) {
										onUpdateTable(cell);
      			...
      			table.addRow(newRow, false,rowCount);
					...
					window.addEventListener("message", (event) => {
						if(event.data.message == "CREATE") {
							...
							loadSelectGrade(message.PRODUCT_GRADES);
							document.getElementById("mainMaterialBoneDryInput").value = Number(mainMaterialBoneDry).toFixed(2);
							apiURL = undefined;
							...
							loadSelectGrade();
							document.getElementById("mainMaterialBoneDryInput").value = Number(mainMaterialBoneDry).toFixed(2);					
							apiURL = message.BASEURL + "/recipeitem/search-for-production-recipe"
