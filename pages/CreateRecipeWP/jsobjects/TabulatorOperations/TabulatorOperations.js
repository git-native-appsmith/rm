export default {
	...
	tabulatorOperations: async () => {
		...
		if(receivedMessage.messageEmitterName === 'VIEW') {
			...
			if(modifStatus == "CO") {
				storeValue('CreateRecipeErrMsg','Modification number status is Closed.',true);
				storeValue('errorFlag',2,true);
				showModal('msgModal');
				return;
			}
			if(modifStatus == "CN") {
				storeValue('CreateRecipeErrMsg','Modification number status is Cancelled.',true);
				storeValue('errorFlag',2,true);
				showModal('msgModal');
				return;
			}
